...
function ListOfNews({news,data}) {
    ...
    const openHandler = (id) =>{
       ...
       const deleteHandler = async () =>{
        try {
            const response = await axios.post(`${serverLink}/delete-news`,{id:openId})
            if(response.data){
                setOpenDelete(false)
                ...
                <div key={i} className='bg-white border-b-[1px] h-24 border-b-neutral-300 dark:border-b-neutral-700 dark:bg-gray-900 p-3 dark:text-white/80'>
                    <div className="flex justify-between gap-3 w-full">
                        <div className="flex w-[30%] md:w-[8%]">
                         <img src={data.files[0]} className='w-16 h-16 rounded-lg '/>
                        </div>
